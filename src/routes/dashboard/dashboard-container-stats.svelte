...
{#if showSkeleton && compact}
	...
	<div class="flex items-center gap-1.5 shrink-0">
		<div class="flex items-center gap-0.5">
			<Play class="size-3 text-muted-foreground/50" />
			<div class="skeleton size-3 rounded"></div>
		</div>
		<div class="flex items-center gap-0.5">
			<Square class="size-3 text-muted-foreground/50" />
			<div class="skeleton size-3 rounded"></div>
		</div>
		<div class="flex items-center gap-0.5">
			<Pause class="size-3 text-muted-foreground/50" />
			<div class="skeleton size-3 rounded"></div>
		</div>
		<div class="flex items-center gap-0.5">
			<RefreshCw class="size-3 text-muted-foreground/50" />
			<div class="skeleton size-3 rounded"></div>
		</div>
		<div class="flex items-center gap-0.5">
			<AlertTriangle class="size-3 text-muted-foreground/50" />
			<div class="skeleton size-3 rounded"></div>
		</div>
		<div class="flex items-center gap-0.5">
			<ArrowUpCircle class="size-3 text-muted-foreground/50" />
			<div class="skeleton size-3 rounded"></div>
		</div>
	</div>
{:else if showSkeleton}
	...
	<div class="grid grid-cols-7 gap-1 min-h-5">
		<div class="flex items-center gap-1">
			<Play class="size-4 text-muted-foreground/50" />
			<div class="skeleton size-4 rounded"></div>
		</div>
		<div class="flex items-center gap-1">
			<Square class="size-4 text-muted-foreground/50" />
			<div class="skeleton size-4 rounded"></div>
		</div>
		<div class="flex items-center gap-1">
			<Pause class="size-4 text-muted-foreground/50" />
			<div class="skeleton size-4 rounded"></div>
		</div>
		<div class="flex items-center gap-1">
			<RefreshCw class="size-4 text-muted-foreground/50" />
			<div class="skeleton size-4 rounded"></div>
		</div>
		<div class="flex items-center gap-1">
			<AlertTriangle class="size-4 text-muted-foreground/50" />
			<div class="skeleton size-4 rounded"></div>
		</div>
		<div class="flex items-center gap-1">
			<ArrowUpCircle class="size-4 text-muted-foreground/50" />
			<div class="skeleton size-4 rounded"></div>
		</div>
		<div class="flex items-center gap-1">
...
{:else if compact}
	...
	<div class="flex items-center gap-1.5 shrink-0">
		...
		<div class="flex items-center gap-0.5" title="Restarting">
			<RefreshCw class="size-3 {containers.restarting > 0 ? 'text-red-500 animate-spin' : 'text-emerald-500'}" />
			<span class="text-2xs font-medium">{containers.restarting}</span>
		</div>
		<div class="flex items-center gap-0.5" title="Unhealthy">
			<AlertTriangle class="size-3 {containers.unhealthy > 0 ? 'text-red-500' : 'text-emerald-500'}" />
		...
		<div class="flex items-center gap-0.5 {containers.pendingUpdates > 0 ? 'pending-glow' : ''}" title="Pending updates">
			<ArrowUpCircle class="size-3 {containers.pendingUpdates > 0 ? 'text-amber-400' : 'text-muted-foreground'}" />
...
{:else}
	...
	<div class="grid grid-cols-7 gap-1 min-h-5">
		...
		<div class="flex items-center gap-1 {containers.pendingUpdates > 0 ? 'pending-glow' : ''}" title="Pending updates">
			<ArrowUpCircle class="size-4 {containers.pendingUpdates > 0 ? 'text-amber-400' : 'text-muted-foreground'}" />
